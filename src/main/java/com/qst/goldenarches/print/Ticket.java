...
public class Ticket implements Printable{
	...
	public Ticket(OrderInfo orderInfo, String printerIp,Setting setting) {
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
		...
		Font font = null;
		int fontSize = 12;
		if("4".equals(orderInfo.getPrintType())) {
			fontSize = 22;
		}
		...
		if(!CollectionUtils.isEmpty(detailInfos)) {
			for (OrderDetailInfo orderDetailInfo : detailInfos) {
				...
				heigth = font.getSize2D();// 字体高度
				g2.drawString(orderDetailInfo.getNumber()+"×"+orderDetailInfo.getProductNo()+" "+orderDetailInfo.getProductName(),(float)x+15,(float)y+line);
				line+=heigth;
			}
		}
		if(!StringUtils.isEmpty(orderInfo.getRemark())) {
			...
			g2.setFont(new Font("SimSun",Font.BOLD,fontSize2));
			g2.drawString(orderInfo.getRemark(),(float)x+15,(float)y+line);
		}
